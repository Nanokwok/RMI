                ...
                <div className='flex flex-row w-full items-center'>
                    ...
                    <div className='flex flex-wrap ml-4 gap-2'>
                        {activeFilter.name.map((filter, index) => (
                            <p key={index} className='bg-slate-50 text-slate-600 px-4 py-2 rounded-md border border-slate-200 font-semibold'>
                                {filter}
                            </p>
                        ))}
                    </div>
                ...
                <div className="flex gap-2 whitespace-nowrap ml-auto">
                    {planDetails.planDetails.map((detail, index) => (
                        <span key={index} className='text-blue-500 text-sm'>
                            Showing {detail.numberOfPlan} plans • {detail.numberOfTasks} tasks • {detail.numberOfOverdue} overdue
                        </span>
                    ))}
                </div>
            </div>
